...
class Workflow(object):
    ...
    def _set_func(path, context, body, engine, target=None, **kwargs):
        ...
        if path.startswith('##'):
            ...
            context_path = ':' + path[1:]
            log.debug(
                "Setting context variable '{path}' to '{data}'".format(
                    **secure_data({'path': context_path, 'data': body_data})))
        ...
        if target:
            ...
            if Workflow._get_path(data, position) != body_data:
                log.debug("Setting '{path}' to '{data}'".format(
                    **secure_data({'path': path, 'data': body_data})))
